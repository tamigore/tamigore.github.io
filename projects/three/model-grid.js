...
export async function buildModelGrid({
	...
	try {
		const [gltf, texturePayload] = await Promise.all([
			...
			Promise.all(
				textureVariants.map(async (variant) => {
					if (!variant.path)
		...
		const modelBounds = { box, size, center, maxDim };
		...
		for (let idx = 0; idx < totalCells; idx++) {
			...
			const labelMeta = labelFactory.getLabelMeta();
			const frontLabelText = variant.label;
			const backLabelText = variant.description;
			const frontLabelMesh = labelFactory.createLabelMesh(frontLabelText);
			const backOffset = labelMeta
				? labelMeta.offset.clone().sub(new THREE.Vector3(-0.45, 0.2, 0))
				: new THREE.Vector3();
			const backLabelMesh = labelFactory.createLabelMesh(backLabelText, {
			...
			scene.add(wrapper);
			wrappers.push({ wrapper, instance, column, row, variant, frontLabel: frontLabelMesh, backLabel: backLabelMesh });
		}
